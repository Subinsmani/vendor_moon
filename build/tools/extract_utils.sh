...
function setup_vendor() {
    ...
    export MOON_ROOT="$3"
    if [ ! -d "$MOON_ROOT" ]; then
        echo "\$MOON_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$MOON_ROOT/$OUTDIR" ]; then
        mkdir -p "$MOON_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$MOON_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$MOON_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$MOON_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$MOON_ROOT"/vendor/moon/build/tools/smali/baksmali.jar
        export SMALIJAR="$MOON_ROOT"/vendor/moon/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$MOON_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$MOON_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$MOON_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$MOON_ROOT"/vendor/moon/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$MOON_ROOT"/"$OUTDIR"/radio
